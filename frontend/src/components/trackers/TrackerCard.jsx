...
export default function TrackerCard({ tracker }) {
  const { quickIncrementProgress, openDrawer, deleteTracker, setSelectedTracker, selectedTracker } =
    useTrackerStore();
    ...
    <div
      ...
      className={`group relative flex flex-col justify-between rounded-2xl border p-4 font-mono transition-all cursor-pointer ${
        isSelected
          ...
          : "border-[var(--border)] bg-[var(--card)] hover:border-[var(--primary)]/50 hover:bg-[var(--hover-bg)] shadow-sm"
      }`}
    >
      <div className="space-y-3">
        ...
        <div className="flex gap-3">
          {tracker.coverUrl ? (
            <img
              ...
              alt={tracker.title}
              className="h-20 w-16 rounded-xl object-cover border border-[var(--border)] shrink-0 shadow-md"
            />
          ) : (
        ...
        <div className="space-y-1.5 pt-1">
          <div className="flex justify-between text-[10px] font-bold text-[var(--text-muted)]">
            <span>
              {unitLabel} {current} / {target}
            </span>
            <span className="text-[var(--primary)] font-extrabold">{percent}%</span>
          </div>
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            onClick={async () => {
              try {
                await useTrackerStore.getState().createTracker({
                  title: `${tracker.title} (Copy)`,
                  description: tracker.description,
                  status: "IN_PROGRESS",
                  currentCount: 0,
                  targetCount: tracker.targetCount || 10,
                  trackerTypeId: tracker.trackerType?.id,
                  categoryId: tracker.category?.id,
                  isFavorite: false,
                });
                alert(`Duplicated "${tracker.title}" successfully!`);
              } catch {
                alert("Failed to duplicate tracker.");
              }
            }}
            className="p-1.5 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-cyan-400 hover:border-cyan-500/40 transition-colors"
