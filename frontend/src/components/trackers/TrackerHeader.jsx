...
export default function TrackerHeader({ onOpenWizard, onOpenMarketplace }) {
  const { filters, setFilters, viewMode, setViewMode, openDrawer } = useTrackerStore();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1">
            Notion + Letterboxd + Habitica + Excel experience for your goals & personal tracking.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2.5 text-xs font-bold text-[var(--text)] hover:border-[var(--primary)]/50 hover:bg-[var(--surface)] transition-all shrink-0 shadow-sm"
          >
            <LayoutTemplate className="h-4 w-4 text-cyan-400" /> Templates
          </button>
          ...
          >
            <Wand2 className="h-4 w-4 animate-bounce" /> Smart Create
          </button>
        </div>
      </div>

      {/* Controls Bar: Search, Type Filter, Status Filter, Favorites, View Switcher */}
